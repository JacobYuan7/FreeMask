...
class Attention(nn.Module):
    ...
    def __init__(
        ...
        if not self.only_cross_attention:
            ...
            # print("self.to_v.weight.shape: ",self.to_v.weight.shape)
        else:
            self.to_k = None
            self.to_v = None
        ...
        #     self.to_k = nn.Linear(cross_attention_dim, inner_dim, bias=bias)
        #     self.to_v = nn.Linear(cross_attention_dim, inner_dim, bias=bias)
        #     self.i_to_k = nn.Linear(768, inner_dim, bias=False)
        #     self.i_to_v = nn.Linear(768, inner_dim, bias=False)
        # ############################# add 0418 ##################################
    ...
    ##################################add0418##################################
    def set_denoising_mode(self, mode=False,layer_n=0):
        self.denoising_mode = mode
        self.layer_n=layer_n
    ###########################################################################
...
class AttnProcessor2_0:
    ...
    def __call__(
        ...
        print("using attention processor 2.0")
        residual = hidden_states
        ...
        if encoder_hidden_states is None:
            ...
            encoder_hidden_states = attn.norm_encoder_hidden_states(encoder_hidden_states)
        # ############add0418#######################
        # if attn.denoising_mode and attn.flag:
        #     img_weight=1
        #     context = encoder_hidden_states 
        #     k = attn.to_k(context[1])
        #     v = attn.to_v(context[1])
        #     if context[0] is not None:
        #         i_k = attn.i_to_k(context[0])
        #         i_v = attn.i_to_v(context[0])
        #         key = torch.cat((k, img_weight * i_k), dim=1)
        #         value = torch.cat((v, img_weight * i_v), dim=1)
