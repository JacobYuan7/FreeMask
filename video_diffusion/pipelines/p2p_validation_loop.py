    ...
    def log_sample_images(
        ...
        for idx, prompt in enumerate(tqdm(self.editing_prompts, desc="Generating sample images")):
            if self.prompt2prompt_edit:
                # print("self.prompt2prompt_edit: ",self.prompt2prompt_edit)
                # print("self.p2p_config{idx}: ",self.p2p_config[idx])
                if self.traverse_p2p_config:
                    p2p_config_now = copy.deepcopy(self.p2p_config[idx])
                else:
                    p2p_config_now = copy.deepcopy(self.p2p_config[idx])
                # print("idx: ",idx)
                # print("self.use_inversion_attention: ",self.use_inversion_attention)
                if idx == 0 and not self.use_inversion_attention:
                    edit_type = 'save'
                    p2p_config_now.update({'save_self_attention': True})
                    #p2p_config_now.update({'save_self_attention': True}) #modified0111
                ...
                #####motion guidance  modified
                p2p_config_now["register_flag"]=register_flag
                register_flag=register_flag+1
                print(" p2p_config_now[register_flag",p2p_config_now["register_flag"])
                #####motion guidance  modified
            ...
            for seed in self.sample_seeds:
                ...
                # print("pipeline in p2p_validation_loop: ",pipeline) #<P2pDDIMSpatioTemporalPipeline>
                ...
                if self.prompt2prompt_edit:
                    ...
                    # sequence2=sequence_return['sdimage_output']
                    attention_output = sequence_return['attention_output']
                    
                else:
                    sequence = sequence_return.images[0]
                torch.cuda.empty_cache() #清除未被使用的cuda缓存

                if self.annotate:
                    images = [annotate_image(image, prompt, font_size=self.annotate_size) for image in sequence]
                else:
                    images = sequence
                if self.make_grid:
        ...
        if self.make_grid:
            ...
            # samples_all = [make_grid(images, cols=1) for images in zip(*samples_all)]#modified
