...
class P2pDDIMSpatioTemporalPipeline(VideoToVideoSDPipeline):
    ...
    def prepare_latents_ddim_inverted(self, image, batch_size, num_images_per_prompt, 
        ...
        ddim_latents_all_step = self.ddim_clean2noisy_loop(init_latents_bcfhw, text_embeddings, self.store_controller)# here register attention
        if store_attention and (save_path is not None) :
            #modified0304
            # os.makedirs(save_path+'/cross_attention',exist_ok=True)
            # # attention_util.show_self_attention_comp(self.store_controller, 16, ["up", "down"],save_path = save_path+'/self_attention')
    ...
    def p2preplace_edit(self, **kwargs): #kwargs里传了config
        ...
        # self.guidance_schedule=self.scheduler.timesteps[0:-2*self.scheduler.order]
        ...
        **args
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(tqdm(timesteps)):
                ...
                # #add motion transfer 0307
                # motion_utils.register_time(self,t)
                # if t in self.guidance_schedule:      #motion transfer code
                #     latents = motion_utils.guidance_step(self,latents, i, t,**args)  #motion transfer code                
                #     motion_utils.change_mode(train=False)
                #     motion_utils.register_batch(self, 2)
