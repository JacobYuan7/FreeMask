...
import datetime
from PIL import Image  # 假设你使用PIL库来处理图片
...
class SpatialBlender:
    ...
    def __call__(self, attention_store, step_in_store: int=None, target_h=None, target_w=None, x_t=None):
        ...
        maps = attention_store["down_cross"][2:4] + attention_store["up_cross"][:3] #range(2,3),range(0,2)
        ...
        target_device = self.alpha_layers.device #(2,1,1,1,1,77)
        ...
        maps = torch.cat(rearranged_maps, dim=1)#(1,100,8,16,16,77)
...
class NewSpatialBlender:
    def get_mask(self, maps, alpha, use_pool, h=None, w=None, x_t=None, step_in_store: int=None, position: int=None):
        if h is None and w is None and x_t is not None:
            h, w = x_t.shape[-2:] # record the shape if only latent is provided
        k = 1
        if maps.dim() == 5: alpha = alpha[:, None, ...]
        ...
        mask = F.interpolate(maps, size=(h, w))
        mask = mask / mask.max(-2, keepdims=True)[0].max(-1, keepdims=True)[0]
        data_np=mask.squeeze(0).detach().cpu().numpy()
        import pandas as pd
        mask = mask.gt(self.th[1-int(use_pool)])
        if self.prompt_choose == 'both':
            assert mask.shape[0] == 2, "If using both source and target prompt"
    ...
    def __call__(self, attention_store, step_in_store: int=None, position: int=None, target_h=None, target_w=None, x_t=None):
        ...
        if is_list:
            ...
            maps = rearranged_item.to(target_device, dtype=target_dtype)
        ...
        if self.prompt_choose == 'source':
            ...
            masked_alpah_layers = self.alpha_layers
        mask = self.get_mask(maps, masked_alpah_layers, True, target_h, target_w, step_in_store=step_in_store,position=position)
